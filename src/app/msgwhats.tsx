...
const SimpleButton: React.FC<ButtonProps> = ({ text, color = 'red' }) => {
  const [count, setCount] = useState(0);

  const phoneNumber = ''; // número com DDI (55) e DDD (ex: 11, 21...) ex: 5511929292929
  const message = 'Olá! Seu documento está prestes a vencer. Verifique por favor!';
  ...
  const handleClick = async () => {
    ...
    if (supported) {
      ...
      alert('WhatsApp não está instalado ou o link é inválido.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Abrir chat WhatsApp</Text>
